...
// 注册响应connect()的回调函数
void tcp_client::register_connect(const cbtype& slot) {
	cbconnect_.connect(slot);
}

// 注册响应receive()的回调函数
void tcp_client::register_receive(const cbtype& slot) {
...
// 注册响应send()的回调函数
void tcp_client::register_send(const cbtype& slot) {
...
// 更新时标
void tcp_client::update_time() {
	timeflag_ = second_clock::local_time().time_of_day().total_seconds();
}

// 尝试连接远程主机
void tcp_client::try_connect(const string host, const int port) {
	peerHost_ = host;
	peerPort_ = port;
...
void tcp_client::start() {
	...
	peerHost_ = ep.address().to_string();
	peerPort_ = ep.port();
...
// 注册回调函数
void tcp_server::register_accept(const cbtype& slot) {
	if (!cbaccept_.empty()) cbaccept_.disconnect_all_slots();
	cbaccept_.connect(slot);
}

// 处理收到连接请求
void tcp_server::handle_accept(const clientptr& client, const boost::system::error_code& ec) {
	if (!ec) {
		if (!cbaccept_.empty()) {
			cbaccept_(client, this);
			client->start();
		}
	}
	start_accept();
...
void tcp_server::start_accept() {
	if (acceptor_.is_open()) {
		clientptr client = boost::make_shared<tcp_client>();	//< 客户端连接
