...
struct ascproto_focus : public ascproto_base {
	...
	double value;			//< ==>转台时为FWHM; ==>相机时为焦点位置
...
public:
	...
	ascproto_object_info &operator=(const ascproto_take_image &proto) {
		...
		delay		= proto.delay;
		frmcnt		= proto.frmcnt;
		priority	= INT_MAX;	// 默认最高优先级, 必须明确中断
		begin_time	= "";
		end_time	= "";
		pair_id		= -1;
		filter 		= proto.filter;
...
public:
	 ...
	 * @return
	 */
	const char* compact_focus(const double value, int& n);
	/*!
...
protected:
	...
	int ibuff_; // 存储区索引. 缓冲区采用一维长度为10*1500=15000字节数组, 通过软件分为10份循环使用
